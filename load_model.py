...
def run_models():
    ...
    print("\nCalculando pesos de clase (class weights)...")
    classes = np.unique(y_train)
    weights = class_weight.compute_class_weight('balanced', classes=classes, y=y_train)
    # Crear el diccionario que Keras espera: {0: peso_clase_0, 1: peso_clase_1, 2: peso_clase_2}
    class_weights_dict = dict(zip(classes, weights))

    print(f"Clases detectadas: {classes}")
    print(f"Pesos calculados (para 0, 1, 2): {class_weights_dict}")
    print("Se aplicarán pesos para penalizar predicciones incorrectas en clases minoritarias.")
