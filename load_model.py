...
from functions import make_forward_return, label_by_fixed_threshold, prepare_xy, compute_class_weights
...
def build_cnn_model(input_shape, params):
    ...
    num_filters = params.get("conv_filters", 32); conv_layers = params.get("conv_layers", 2);
    activation = 'tanh'
    for _ in range(conv_layers): model.add(Conv1D(filters=num_filters, kernel_size=3, activation=activation, padding='same')); model.add(MaxPooling1D(pool_size=2));
    num_filters *= 2
    dense_units = params.get("dense_units", 64); model.add(Flatten()); model.add(Dense(dense_units, activation=activation));
...
def build_mlp_model(input_shape, params):
    model = Sequential(); model.add(Input(shape=(input_shape,))); dense_layers = params.get("dense_layers", 2); units = params.get("dense_units", 128);
    activation = 'relu'
    for _ in range(dense_layers): model.add(Dense(units, activation=activation))
    model.add(Dense(3, activation='softmax')); optimizer = "adam";
...
def run_models():
    ...
    CON class_weight. VWAP eliminado. Dropna específico. Incluye Data Drift Analysis.
    """
    ...
    if datos.empty: 
        print(f"Error: DataFrame vacío después de make_forward_return(horizon={horizon}).")
        return
    ...
    if train_df.empty or test_df.empty or validation_df.empty: 
        print("Error: DataFrames vacíos después de split_dfs.")
        return
    ...
    print("Etiquetando train_df...");
    train_df = label_by_fixed_threshold(train_df, alpha)
    print("Etiquetando test_df..."); test_df = label_by_fixed_threshold(test_df, alpha)
    print("Etiquetando validation_df...");
    validation_df = label_by_fixed_threshold(validation_df, alpha)

    # --- 4. Definir Columnas de Features y Target ANTES de dropna ---
    exclude_cols = ["Date", "fwd_ret", "target", "Open", "High", "Low", "Close", "Volume", "Adj Close", "Datetime", "vwap_dev"]
    potential_feature_cols = [c for c in train_df.columns if c not in exclude_cols]
    feature_cols = [col for col in potential_feature_cols if train_df[col].notna().any()]
    ...
    if train_df.empty: 
        print("Error: train_df vacío después de dropna específico.")
        return
    ...
    if any(s is None for s in scalers): 
        print("Error: Falló entrenamiento de scalers.")
        return 
    ...
    if train_scaled.empty or test_scaled.empty or val_scaled.empty: 
        print("Error: DataFrames vacíos después de escalar.")
        return 
    print("Guardando datos escalados en CSV para main.py...")
    ...
    drift_val_results = analyze_feature_drift(train_scaled, val_scaled, 
    feature_cols, alpha=drift_alpha)
    print("--- Análisis de Data Drift Completado ---")
    

    # --- 8. Preparar Datos X, y ---
    print("\nPreparando datos X/y para Keras...")
    try:
        X_train, X_val, X_test, y_train_oh, y_val_oh, y_test_oh, feature_cols_from_prep = prepare_xy(
    ...
    class_weights_dict = compute_class_weights(y_train_oh)


    # --- 9. Definir Configuraciones y Callbacks ---
    ...
    # --- 10. Entrenar Modelos MLP ---
    print("\n--- Entrenando Modelos MLP ---")
    mlflow.set_experiment("MLP_Models_WYNN_FixedAlpha_H1_WithWeight") 
    ...
    for config in configuraciones_mlp:
        ...
        with mlflow.start_run(run_name=nombre_run) as run:
            ...
            mlflow.log_param("activation", "relu"); mlflow.log_param("optimizer", "adam"); mlflow.set_tag("tipo_modelo", "MLP")
            modelo_mlp = build_mlp_model(forma_entrada_mlp, config)
            
            modelo_mlp.fit( X_train, y_train_oh, validation_data=(X_val, y_val_oh), 
                            epochs=50, batch_size=32, callbacks=[callback_parada], 
                            verbose=6, class_weight=class_weights_dict 
            ) 
            mlflow.tensorflow.log_model(modelo_mlp, "model", registered_model_name=nombre_run)

    # --- 11. Entrenar Modelos CNN ---
    print("\n--- Entrenando Modelos CNN ---")
    mlflow.set_experiment("CNN_Models_WYNN_FixedAlpha_H1_WithWeight") 
    ...
    for config in configuraciones_cnn:
        ...
        with mlflow.start_run(run_name=nombre_run) as run:
             print(f"Entrenando: {nombre_run}")
             mlflow.log_params(config);
             mlflow.log_param("activation", "tanh"); mlflow.log_param("optimizer", "adam"); mlflow.set_tag("tipo_modelo", "CNN")
             modelo_cnn = build_cnn_model(forma_entrada_cnn, config)
             
             modelo_cnn.fit( X_train_cnn, y_train_oh, validation_data=(X_val_cnn, y_val_oh), 
                             epochs=50, batch_size=32, callbacks=[callback_parada], 
                             verbose=6, class_weight=class_weights_dict
             )
