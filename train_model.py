...
def run_training_pipeline():
    ...
    Usa lógica de cuantiles (q=0.2, q=0.8) y horizonte 1.
    """
    # --- 1. Carga y Features ---
    ...
    # Usamos la función de fechas del compañero
    datos = fechas(datos) 
    # Usamos la función de features del compañero
    ...
    horizon = 1
    # Usamos make_forward_return del compañero
    ...
    # Calcular umbrales SÓLO en TRAIN
    ...
    # Identificar features (columnas que NO están en base_exclude)
    # Esto ahora incluirá las features de 'features.py' Y 'vwap_dev'
    feature_cols = [c for c in train_df.columns if c not in base_exclude]
    
    # Columnas a revisar para NaNs (features + target + OHLCV para el scaler)
    ...
    # --- 4. Entrenar y Aplicar Scalers (Lógica del compañero) ---
    print("Entrenando scalers (4-scalers) en train_df...")
    
    # --- CORRECCIÓN INICIA ---
    # 1. Desempaquetar los 4 scalers de la tupla
    mm_scaler, rb_scaler, st_scaler, ohlcv_scaler = fit_scalers(train_df)
    
    # 2. Revisar si el entrenamiento falló
    ...
    # 3. Pasar los 4 scalers como argumentos a apply_scalers
    ...
    drift_alpha = 0.05 
    # Usamos la lista de features que 'prepare_xy' usará
    ...
    try:
        # Usamos prepare_xy del compañero
    ...
    # Usamos compute_class_weights del compañero
    ...
    mlflow.set_experiment("MLP_Models_WYNN_Quantile_H1") 
    ...
    mlflow.set_experiment("CNN_Models_WYNN_Quantile_H1") 
    X_train_cnn = np.expand_dims(X_train, axis=-1)
    X_val_cnn = np.expand_dims(X_val, axis=-1)
    
    forma_entrada_cnn = X_train_cnn.shape[1]
    for config in configuraciones_cnn:
        nombre_run = f"CNN_conv{config['conv_layers']}_filters{config['conv_filters']}_dense{config['dense_units']}_relu_Weighted"
        with mlflow.start_run(run_name=nombre_run) as run:
             print(f"Entrenando: {nombre_run}")
             mlflow.log_params(config)
             mlflow.log_param("activation", "relu")
